...
function Categories() {
  ...
  const addCategory = async () => {
    ...
    if (!name) return setError("Le nom ne peut pas être vide");
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {categories.map((cat) => (
          <div key={cat.id} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700 rounded">
            {editing?.id === cat.id ? (
              <div className="flex gap-2 w-full">
                <input
                  ...
                  autoFocus
                  onKeyPress={(e) => { if (e.key === "Enter") updateCategory(); if (e.key === "Escape") cancelEdit(); }}
                />
                <button onClick={updateCategory} className="bg-green-600 px-2 rounded hover:bg-green-700 text-white">
                  <Check className="w-4 h-4" />
                </button>
                <button onClick={cancelEdit} className="bg-gray-500 px-2 rounded hover:bg-gray-600 text-white">
                  <X className="w-4 h-4" />
                </button>
              </div>
            ) : (
              <>
                ...
                <div className="flex gap-2">
                  <button onClick={() => startEdit(cat)} className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
                    <Edit className="w-4 h-4" />
                  </button>
                  <button onClick={() => removeCategory(cat)} className="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300">
