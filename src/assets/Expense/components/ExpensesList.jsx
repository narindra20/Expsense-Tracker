...
function ExpensesList({ expenses = [], onDelete, onUpdate, isDarkMode }) {
  ...
  const badgeClass = (type) => {
    if (type.toLowerCase() === "ponctuelle") {
      return isDarkMode ? "bg-green-700 text-green-100 px-2 rounded-full text-xs" : "bg-green-100 text-green-800 px-2 rounded-full text-xs";
    } else {
      return isDarkMode ? "bg-blue-700 text-blue-100 px-2 rounded-full text-xs" : "bg-blue-100 text-blue-800 px-2 rounded-full text-xs";
    }
  };
      ...
      <div className={containerClass}>
        <table className="min-w-full">
          <thead className={tableHeadClass}>
            <tr>
              {["Titre", "Catégorie", "Date", "Type", "Montant", "Actions"].map((title) => (
              ...
              <tr key={expense.id}>
                {editId === expense.id ? (
                  <>
                    ...
                    <td className="px-6 py-4"><span className={badgeClass(expense.type)}>{expense.type}</span></td>
                  ...
                  <>
                    ...
                    <td className="px-6 py-4 flex gap-2 items-center">
                      ...
                      <button onClick={() => downloadReceipt(expense)} className="flex items-center gap-1 text-green-600 hover:text-green-900">
